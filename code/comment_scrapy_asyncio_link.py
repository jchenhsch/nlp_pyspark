...
async def main(video_links, keyword='chinese economy'):
    ...
    # Create tasks for each video link and await their completion
    tasks = [scrape_comments_wrapper(video_url) for video_url in video_links]
    await asyncio.gather(*tasks)

# List of video links to scrape comments from
keywords = ["Financial Advice","Books","Relationship"]

# Loop through the list of keywords to get the comments
for keyword in keywords:
    video_links = asyncio.get_event_loop().run_until_complete(get_youtube_search_links(keyword))
    sublists = [video_links[i:i + 10] for i in range(0, len(video_links), 10)]
